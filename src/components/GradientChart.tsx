...
export const GradientChart: React.FC<GradientChartProps> = ({
  ...
  useEffect(() => {
    ...
    if (data.gradientNorms.length < 1) {
      // Show "No data" message
      ctx.fillStyle = '#6b7280';
      ctx.font = '16px system-ui';
      ctx.textAlign = 'center';
      ctx.fillText('Gradient norms will appear during optimization', width / 2, height / 2);
      return;
    }

    // Find data ranges
    const minValue = Math.min(...data.gradientNorms);
    const maxValue = Math.max(...data.gradientNorms);
    const valueRange = Math.max(maxValue - minValue, 0.001); // Avoid division by zero

    // Scale functions
    const scaleX = (i: number) => margin.left + (i / Math.max(data.gradientNorms.length - 1, 1)) * plotWidth;
    ...
    if (data.gradientNorms.length > 1) {
      ...
      data.gradientNorms.forEach((value, i) => {
        const x = scaleX(i);
      ...
      ctx.lineTo(scaleX(data.gradientNorms.length - 1), margin.top + plotHeight);
    ...
    // Draw current point
    if (data.currentIteration > 0 && data.currentIteration < data.gradientNorms.length) {
      const currentValue = data.gradientNorms[data.currentIteration];
    ...
    for (let i = 0; i <= numXTicks; i++) {
      const x = margin.left + (i / numXTicks) * plotWidth;
      const iteration = Math.round((i / numXTicks) * Math.max(data.gradientNorms.length - 1, 0));
      ctx.textAlign = 'center';
    ...
    for (let i = 0; i <= numYTicks; i++) {
      ...
      ctx.textAlign = 'right';
      ctx.fillText(value.toFixed(4), margin.left - 15, y + 4);
    }
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="ml-auto text-sm text-gray-500">
          Current: {data.gradientNorms[data.gradientNorms.length - 1]?.toFixed(4) || '0.0000'}
        </div>
      </div>
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>Gradient norm shows the magnitude of the estimated gradient, indicating convergence when it approaches zero.</p>
      </div>
    </div>
  );
};